...
export default function BulkModal({
  ...
  const handleConfirmSubmit = async (e) => {
    ...
    const { amount, outputAddress } =
      stakeData.current;
    const nodes
    serviceURI, operatorPublicKey, selectedChains, amount?, outputAddress?
    if stakeData.amountChecked is true
      amount = stakeData.amount
      else amount = 0

    if stakeData.outputAddressChecked is true
      outputAddress = stakeData.outputAddress
      else outputAddress = ""
      ...
      <StakingModalContent>
        ...
        <form onSubmit={(e) => handleConfirmSubmit(e)}>
          ...
          <h2 className="you-are-sending">
            You are Staking {stakeData.current?.amountTotal} POKT across {stakeData.nodes.length} nodes
          </h2>
